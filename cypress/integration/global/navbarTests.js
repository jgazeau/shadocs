const testCases = ['desktop','mobile']
  ...
  describe('for: navbar' + ' on ' + testCase, () => {
    ...
    describe('when multilingual dropdown', () => {
      it('hover should display languages', () => {
        cy.get(`#langsSelectorContainer${extendId}`)
          .then(($elem) => {
            $elem.get(0).classList.toggle('is-hovered', true)
          })
        cy.get(`#dropdown-menu-langs${extendId}`)
          .should('be.visible')
      })
      it('should have 2 languages', () => {
        cy.get(`#dropdown-menu-langs${extendId} .dropdown-item`)
