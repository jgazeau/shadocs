...
document.addEventListener('DOMContentLoaded', function() {
  ...
  for (let i = 0; i < ifei.length; i++) {
    ifei[i].addEventListener('mouseenter', function(e) {
      ...
      if (e.relatedTarget !== entriesContainer) {
        ...
        let sidebarCurrentWidth = parseFloat(getComputedStyle(sidebarContainer).getPropertyValue('width'));
        let sidebarMinimumWidth = parseFloat(getComputedStyle(document.getElementById('sidebarMobileWrapper')).getPropertyValue('max-width'))
        if (sidebarContainer.classList.contains('is-sidebar-collapsed') && entriesContainer && (sidebarCurrentWidth <= sidebarMinimumWidth)) {
          entriesContainer.classList.toggle('is-hovered', true);
          entriesContainer.classList.toggle('is-invisible', true);
          let sidebar = document.getElementById('sidebar');
          let entriesWrapper = entriesContainer.getElementsByClassName('is-sidebar-list-wrapper')[0];
          ...
          if (!isInScrollContainer(this, sidebar)) {
            ...
            if (scrollContainer) {
              scrollContainer.scrollTop = scrollContainer.scrollTop + getScrollOffset(this, scrollContainer);
            };
          };
          const sidebarPadding = parseFloat(getComputedStyle(sidebar).getPropertyValue('padding-top'));
          const sidebarBorder = parseFloat(getComputedStyle(sidebar).getPropertyValue('border-right-width'));
          const logoBorder = parseFloat(getComputedStyle(document.getElementById('globalLogo')).getPropertyValue('margin-bottom'));
          const sidebarTop = sidebarContainer.getBoundingClientRect().top - logoBorder;
          const sidebarIconTop = this.getBoundingClientRect().top - sidebarPadding - sidebarBorder;
          ...
          entriesContainer.style.left = this.getBoundingClientRect().right + "px";
    ...
    ifei[i].addEventListener('mouseleave', function(e) {
      ...
      let entriesContainer = this.nextElementSibling;
      let entriesWrapper = entriesContainer.getElementsByClassName('is-sidebar-list-wrapper')[0];
      if (sidebarContainer.classList.contains('is-sidebar-collapsed') && entriesContainer) {
        if (e.relatedTarget !== entriesContainer) {
  ...
  for (let i = 0; i < ie.length; i++) {
    ie[i].addEventListener('mouseleave', function(e) {
      if (e.relatedTarget && getFirstAncestorByClass(e.relatedTarget, 'is-fs-expandable-icon') !== this.previousElementSibling) {
        let sidebarContainer = document.getElementById('sidebarContainer');
        if (sidebarContainer.classList.contains('is-sidebar-collapsed')) {
          ...
          this.style.bottom = "";
          this.style.left = "";
          this.getElementsByClassName('is-sidebar-list-wrapper')[0].style.maxHeight = "";
        };
      };
    });
  };
...
function manageActiveEntryScroll(e) {
  if (e && isInViewport(e)) {
    ...
    if (scrollContainer) {
      scrollContainer.scrollTop = scrollContainer.scrollTop + getScrollMiddleOffset(e, scrollContainer);
    };
  };
};