...
(function () {
    ...
    for (const line of input.split('\n')) {
      ...
      if (indexes.length > 0) {
        ...
        for (let i = minIndex; i < maxIndex + 1; i++) {
          ...
          if (
            (tIndex && i <= tIndex[1] + maxCharOffset) ||
            (nIndex && i >= nIndex[0] - maxCharOffset)
          ) {
  ...
  function getDiacriticsRegex(input) {
    ...
    for (const inputChar of input) {
      ...
      diacriticsLoop: for (
        let j = 0;
        j < defaultDiacriticsRemovalMap.length;
        j++
      ) {
