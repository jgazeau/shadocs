...
Par défaut des raccourcis clavier sont fournis pour permettre d'interagir plus facilement avec le thème, mais il est possible de définir des raccourcis spécifique par le biais d'un minimum de configuration.  
...
Afin de définir un raccourci, il faut suivre le processus suivant (exemple avec le raccourci **example** (`Maj + 1`) du site du thème):
...
/>}}
2. Définir les libellés du raccourci qui apparaîtront dans le menu d'aide des raccourcis (dans le fichier de traduction *i18n* adéquat):
{{< snippet
